...
class NumbersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create number" do
    assert_difference('Number.count') do
      post numbers_url, params: { number: { description: @number.description, name: @number.name } }
    end
  ...
  test "should update number" do
    patch number_url(@number), params: { number: { description: @number.description, name: @number.name } }
